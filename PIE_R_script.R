...
get_phage_bact_coverages <- function(name_of_file) {
  testPhage <- read.delim(name_of_file, header=FALSE, sep=":", stringsAsFactors=FALSE)
...
outpath<-myArgs[4]
sample_name<-myArgs[5]

rebels<-bootstrapping(file_name,plot_name,threshold_number)

write.csv(rebels, paste(outpath,"/",sample_name,"_phages_exceeding_threshold.csv",sep=""), row.names = FALSE)
