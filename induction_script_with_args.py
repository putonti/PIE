...
def categorize_assembled_contigs(contigs, phage_sequences):
    ...
    command = path_of_tools + 'ncbi-blast-2.12.0+/bin/blastn -query ' + contigs + ' -db ' + phage_name + ' -max_target_seqs 1 -outfmt="10 qseqid sseqid qcovs qstart qend pident length evalue bitscore" -out ' + phage_name + '_blastn.csv'
    os.system(command)
    ...
    for i in blast_results:
        ...
        if pair in top_hits.keys():
            ...
            if float(i['bitscore']) > float(b):
                top_hits[pair] = (float(i['qcovs']), float(i['pident']), float(i['bitscore']), float(i['length']), int(i['qstart']), int(i['qend']))
        else:
            top_hits[pair] = (float(i['qcovs']), float(i['pident']), float(i['bitscore']), float(i['length']), int(i['qstart']), int(i['qend']))
